...
final class DayRangeIndicatorView: UIView {
    ...
    override func draw(_ rect: CGRect) {
        ...
        for (index,dayRowFrame) in dayRowFrames.enumerated() {
            ...
            if index == 0 {
                let path = UIBezierPath(roundedRect: dayRowFrame, byRoundingCorners: [.bottomLeft, .topLeft], cornerRadii: CGSize(width: 20.0, height: 20.0))
                context?.addPath(path.cgPath)
                context?.fillPath()

            } else if index == dayRowFrames.count - 1 {
                let customDayframe = CGRect(x: dayRowFrame.origin.x, y: dayRowFrame.origin.y, width: dayRowFrame.width - 30, height: dayRowFrame.height)

                let path = UIBezierPath(roundedRect: customDayframe, byRoundingCorners: [.bottomRight, .topRight], cornerRadii: CGSize(width: 20.0, height: 20.0))
                context?.addPath(path.cgPath)
                context?.fillPath()
            } else {
                let roundedRectanglePath = UIBezierPath(roundedRect: dayRowFrame, cornerRadius: CGFloat(cornerRadius))
